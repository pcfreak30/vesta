...
rebuild_web_domain_conf() {
    ...
    add_web_config "${WEB_SYSTEM}" "${domain_idn}" "${WEB_SYSTEM}" "$TPL.tpl"
    ...
    if [ ! -z "$PROXY_SYSTEM" ] && [ ! -z "$PROXY" ]; then
        ...
        add_web_config "${PROXY_SYSTEM}" "${domain_idn}" "${PROXY_SYSTEM}" "$PROXY.tpl"
        if [ "$SSL" = 'yes' ]; then
            conf="$HOMEDIR/$user/conf/web/${PROXY_SYSTEM}_${domain_idn}_ssl.conf"
            add_web_config "${PROXY_SYSTEM}" "${domain_idn}" "${PROXY_SYSTEM}" "$PROXY.stpl"
        fi
    fi
